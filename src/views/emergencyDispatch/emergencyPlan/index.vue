...
const searchFormParams = reactive<emergencyListRes>({
  title: "",
  riskType: ""
});
...
const archiveListFun = async () => {
  ...
  const { data } = await emergencyList(toRaw(searchFormParams)).finally(() => {
...
const opRow = ref<addemergencyRes>();
function openDialog(type: "add" | "update", row?: renewemergencyRes) {
  opType.value = type;
  opRow.value = row;
...
const openDetailDialog = row => {
  detailFromModalRef.value.handleOpened(row.emergencyId);
};
...
function resetForm() {
  searchFormParams.title = "";
  searchFormParams.riskType = "";
