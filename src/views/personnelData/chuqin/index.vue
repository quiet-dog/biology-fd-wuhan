      ...
      <template v-slot="{ size, dynamicColumns }">
        ...
        >
          <template #enterStatus="{ row }">
            <el-checkbox
              v-model="row.isChuQin"
              @change="status => changeStatus(status, row)"
              :label="row.enterStatus === '通行' ? '是' : '否'"
              size="large"
            />
          </template>
          <template #doorDate="{ row }">
            <span>{{
              row.doorDate != 0 && row.doorDate != null
                ? dayjs(row.doorDate).format("YYYY-MM-DD HH:MM:ss")
                : ""
            }}</span>
          </template>
        </pure-table>
