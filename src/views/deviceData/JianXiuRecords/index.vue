  ...
  <div class="main">
    ...
    >
      ...
      <el-form-item label="检修人员：">
        <el-input
          class="!w-[200px]"
          v-model="searchFormParams.inspector"
          placeholder="请输入检修人员"
          clearable
        />
      </el-form-item>
...
const timeRange = computed<[string, string] | null>({
  get() {
    if (
      searchFormParams.startInspectionDate &&
      searchFormParams.endInspectionDate
    ) {
      return [
        searchFormParams.startInspectionDate,
        searchFormParams.endInspectionDate
  ...
  set(v) {
    if (v?.length === 2) {
      searchFormParams.startInspectionDate = v[0];
      searchFormParams.endInspectionDate = v[1];
    } else {
      searchFormParams.startInspectionDate = undefined;
      searchFormParams.endInspectionDate = undefined;
    }
  }
});
...
const searchFormParams = reactive<inspectionRes>({
  equipmentCode: "",
  inspector: "",
...
const archiveListFun = async () => {
  ...
  const { data } = await inspectionList(toRaw(searchFormParams)).finally(() => {
...
const detailRef = ref();
const openDetailDialog = (row: renewinspectionRes) => {
