...
<template>
  <v-dialog show-full-screen :fixed-body-height="false" use-body-scrolling title="采样历史记录" v-model="visible"
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="消杀时间：">
            <el-date-picker @change="changeDate" v-model="datePicker" value-format="YYYY-MM-DD" type="daterange"
        ...
        <el-col :span="4">
          <el-button style="float: right;" type="warning" @click="exportClick">导出</el-button>
        </el-col>
      </el-row>
    </el-form>
    ...
    <el-pagination style="float: right;margin-top: 10px;" @size-change="getList" @current-change="getList" background
      layout="prev, pager, next" v-model:page-size="pageInfo.pageSize" v-model:current-page="pageInfo.pageNum"
      :total="total" />
  </v-dialog>
</template>
