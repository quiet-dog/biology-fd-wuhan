...
function loadModel() {
  if (
    searchPersonnel.value.pageNum * searchPersonnel.value.pageSize >
    personnelSelect.value.total
  ) {
    return;
  }
...
const deptName = computed(() => {
  let dept = "";
  if (
    Array.isArray(personnelSelect.value.data) &&
    personnelSelect.value.data.length > 0
  ) {
...
<template>
  <v-dialog
    ...
    use-body-scrolling
    :title="type == 'add' ? '添加心理健康档案' : '更新心理方案'"
    v-model="visible"
    :loading="loading"
  ...
  >
    <el-form :model="formData" label-width="100px" :rules="rules" ref="formRef">
      <el-row>
        <el-col :span="12">
          <el-form-item label="人员名称：" prop="personnelId">
            ...
            <el-select v-model="formData.personnelId">
              <div v-infinite-scroll="loadModel">
                <el-option
                  v-for="item in personnelSelect.data"
