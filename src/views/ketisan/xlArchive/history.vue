...
function getShuJu() {
  resultShiJuanList({
    ...
    userId: searchShuJu.value.userId != 0 ? searchShuJu.value.userId : undefined,
...
function getResult() {
  resultShiJuanList({
    ...
    userId: searchResult.value.userId != 0 ? searchShuJu.value.userId : undefined,
    isNeedLastTime: true,
  ...
  <VDialog @cancel="cancel" v-model="visiable" title="心理健康信息">
    <ElTabs>
      <ElTabPane label="心理测评数据">
        ...
        <el-pagination @current-change="getShuJu" v-model:page-size="searchShuJu.pageSize" v-model:current-page="searchShuJu.pageNum" layout="prev, pager, next,total" :total="searchShuJu.total" />
      </ElTabPane>
      <ElTabPane label="干预措施">
        ...
        <el-pagination @current-change="getResult" v-model:page-size="searchResult.pageSize" v-model:current-page="searchResult.pageNum" layout="prev, pager, next,total" :total="searchShuJu.total" />
      </ElTabPane>
    </ElTabs>
  </VDialog>
</template>
