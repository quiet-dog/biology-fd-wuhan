...
const formRef = ref();
const { form, loading, onSearch, resetForm, columns, dataList, pagination, handlePageChange, dialogOb,playUrl } = useVideoHook();
const channelId = ref("")
...
function viewVideo(row) {
  ...
  getStreamUrlApi(row.channelid).then(res => {
...
<template>
  <div class="main">
    <el-form :inline="true" class="search-form bg-bg_color w-[99/100] pl-8 pt-[12px]" :model="form">
    ...
    <PureTableBar title="监控列表" :columns="columns" @refresh="onSearch">
      ...
      <template v-slot="{ size, dynamicColumns }">
        <pure-table border align-whole="center" showOverflowTooltip table-layout="auto" :loading="loading" :size="size"
          adaptive :data="dataList" :columns="dynamicColumns" :pagination="pagination"
          :paginationSmall="size === 'small' ? true : false" :header-cell-style="{
            background: 'var(--el-table-row-hover-bg-color)',
            color: 'var(--el-text-color-primary)'
          }" @page-current-change="handlePageChange">
          <template #operation="{ row }">
            <el-button class="reset-margin" link type="primary" :size="size" @click="viewVideo(row)" :icon="useRenderIcon(EditPen)">
              查看
            </el-button>
          </template>
        </pure-table>
    ...
    <ElDialog :title="dialogOb.title" @opened="opened" width="960px" style=" width: 1060px;
  height: 697px;" destroy-on-close	 v-model="dialogOb.visible">
